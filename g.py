...
class AsyncCli:
            ...
            try:
                ...
                if self.writer2 and not self.writer2.is_closing():
                    ...
                    if heartbeat_count % 12 == 0:
                        print(f'💓 {self.u} نبضات القلب نشطة')
                        
            except Exception as e:
                print(f'⚠️ Heartbeat error {self.u}: {e}')
                break
    ...
    async def _run_with_keepalive(self):
        ...
        while True:
            try:
                ...
                print(f'⚠️ {self.u} انقطع الاتصال: {e}')
                self.alive = False
                ...
                print(f'🔄 {self.u} محاولة إعادة الاتصال #{self.reconnect_attempts}')
                
                if self.reconnect_attempts > 10:
                    print(f'❌ {self.u} فشل الاتصال بعد 10 محاولات')
                    break
    ...
    async def _run(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            self.alive = True
            self.reconnect_attempts = 0
            print(f'✅ {self.u} متصل بنجاح واستمراريته مضمونة')
            ...
            while self.alive:
                try:
                    ...
                    if not data:
                        print(f'⚠️ {self.u} لا توجد بيانات، اتصال مقطوع؟')
                        break
                except asyncio.TimeoutError:
                    continue
                except ConnectionResetError:
                    print(f'⚠️ {self.u} تم إعادة تعيين الاتصال')
                    break
                except Exception as e:
                    print(f'⚠️ {self.u} خطأ في القراءة: {e}')
                    break
...
def load_accounts():
    ...
    if _accounts_loaded:
        print("⚠️ الحسابات تم تحميلها مسبقاً ولا يمكن تحميلها مرة أخرى")
        return len(_clis)
    
    if _accounts_connecting:
        print("⚠️ جاري تحميل الحسابات حالياً...")
        return 0
    ...
    try:
        ...
        print(f"\n🚀 جاري تحميل {len(accs)} حساب (مرة واحدة فقط)...")
        ...
        for u, p in accs.items():
            ...
            if not existing:
                ...
                cli.start()
                _clis.append(cli)
                time.sleep(0.3)  # تأخير بسيط - استخدم time.sleep بدلاً من await
            else:
                print(f"⚠️ الحساب {u} موجود مسبقاً")
        
        _accounts_loaded = True
        _accounts_connecting = False
        
        print(f"\n🎉 تم تشغيل جميع الحسابات بنجاح! ({len(_clis)} حساب)")
        print("💎 الحسابات جاهزة للهجوم عند الطلب ولن يتم إعادة تحميلها")
        print("🔄 النظام سيبقيها متصلة بشكل دائم\n")
        return len(_clis)
    ...
    while not stop_event.is_set():
        ...
        for client in active:
            ...
            try:
                ...
                print(f'MASJON error على {client.u}: {e}')
...
╔══════════════════════════════════════════════════════════╗
║              MASJON BOT - API JSON                       ║
║      اتصال واحد فقط - الحسابات تبقى متصلة دائماً         ║
╚══════════════════════════════════════════════════════════╝
    """)
    ...
    print("📂 جاري تحميل الحسابات من accs.json...")
    try:
        with open(accs_file, "r") as f:
            accs = json.load(f)
        print(f"📊 تم العثور على {len(accs)} حساب في الملف")
    except:
        print("⚠️ accs.json غير موجود أو فارغ")
    ...
    print(f"\n{'='*60}")
    print(f"✅ النظام جاهز! {len(_clis)} حساب متصل")
    print(f"💚 البوتات النشطة: {len(get_active_bots())}")
    print(f"🔄 نظام إعادة الاتصال التلقائي: مفعل")
    print(f"💓 نبضات القلب: كل 25 ثانية")
    print(f"\n🌐 API يعمل على: http://0.0.0.0:5000/")
    print(f"📊 لمشاهدة الإحصائيات: http://0.0.0.0:5000/stats")
    print(f"\n💡 ملاحظة: الحسابات تم تحميلها مرة واحدة فقط")
    print(f"   وإذا انقطع أي حساب سيتم إعادة اتصاله تلقائياً")
    print(f"{'='*60}\n")
